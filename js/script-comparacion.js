...
const selectProducto1 = document.querySelector("#producto1");
const selectProducto2 = document.querySelector("#producto2");

// Carga los productos en los elementos select
function cargarProductosEnSelects() {
    productos.forEach(producto => {
        // Crea una nueva opción para cada producto
        const opcion1 = document.createElement("option");
        const opcion2 = document.createElement("option");

        // Establece el valor y el texto de las opciones
        opcion1.value = producto.id;
        opcion1.textContent = producto.nombre;
        opcion2.value = producto.id;
        opcion2.textContent = producto.nombre;

        // Añade las opciones a los elementos select
        selectProducto1.append(opcion1);
        selectProducto2.append(opcion2);
    });
}
...
// Obtén el formulario
const formComparacion = document.querySelector("#form-comparacion");
const tablaComparacion = document.querySelector("#tabla-comparacion");
...
formComparacion.addEventListener("submit", function(e) {
    ...
    const idProducto1 = selectProducto1.value;
    const idProducto2 = selectProducto2.value;

    // Encuentra los productos correspondientes
    const producto1 = productos.find(producto => producto.id === idProducto1);
    const producto2 = productos.find(producto => producto.id === idProducto2);

    const tablaComparacion = document.querySelector("#tabla-comparacion");

    // Verifica si los productos han sido seleccionados
    if (idProducto1 == "Texto Ayuda" || idProducto2 == "Texto Ayuda") {
        const tablaComparacion = document.querySelector("#tabla-comparacion");
        if (tablaComparacion) {
            tablaComparacion.remove();
            if (idProducto1 == "Texto Ayuda" && idProducto2 == "Texto Ayuda") {
                const resultadoComparacion = document.querySelector("#resultado-comparacion");
                resultadoComparacion.innerHTML = `<p>No seleccionaste ningún producto</p>`;
            } else {
                const resultadoComparacion = document.querySelector("#resultado-comparacion");
                resultadoComparacion.innerHTML = `<p>Por favor, selecciona dos productos para comparar.</p>`;
            }
        }
        if (idProducto1 == "Texto Ayuda" && idProducto2 == "Texto Ayuda") {
            mensaje.textContent = "No seleccionaste ningún producto";
        } else {
            mensaje.textContent = "Por favor, selecciona dos productos para comparar.";
        }
    } else {
        ...
        <tbody class="col-12">
            ...
            <tr>
                ...
                <td class="col-6">Valor Energetico(Kcal): ${producto2.energia}</td>
            </tr>
            <tr>
                <td class="col-6">Proteinas: ${producto1.proteinas}</td>
                <td class="col-6">Proteinas: ${producto2.proteinas}</td>
            </tr>
            <tr>
                <td class="col-6">CarboHidratos: ${producto1.carbohidratos}</td>
                <td class="col-6">CarboHidratos: ${producto2.carbohidratos}</td>
            </tr>
            <tr>
                <td class="col-6">Grasas: ${producto1.grasas}</td>
                <td class="col-6">Grasas: ${producto2.grasas}</td>
            </tr>
        </tbody>
        </table>
        `;
